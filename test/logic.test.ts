...
describe('Food tests', () => {
  ...
  it.only('hazardFood1: seeks out food when stuck in hazard with nowhere else to go', () => {
    debugger
    ...
    const moveResponse: MoveResponse = move(gameState)
    expect(moveResponse.move).toBe("up") // have just enough health to reach food at (2,6). Right may maximize board space but it depends on another food spawning to pay off
  })
})
